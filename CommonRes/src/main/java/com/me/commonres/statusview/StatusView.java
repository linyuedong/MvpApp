package com.me.commonres.statusview;
...
public class StatusView extends FrameLayout {
    ...
    public StatusView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.LoadingLayout, 0, 0);
        for (int i = 0; i < ta.getIndexCount(); i++) {
            int attr = ta.getIndex(i);
            if (attr == R.styleable.LoadingLayout_loading_layout) {
                loadingLayoutId = ta.getResourceId(attr, loadingLayoutId);
            } else if (attr == R.styleable.LoadingLayout_empty_layout) {
                emptyLayoutId = ta.getResourceId(attr, emptyLayoutId);
            } else if (attr == R.styleable.LoadingLayout_error_layout) {
    ...
    public void setRetry(int viewId, boolean isShowRetry, String retryText,
                         OnClickListener listener, ViewHolder viewHolder) {
        if (isShowRetry) {
